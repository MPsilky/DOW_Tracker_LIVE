...
def price_at_or_before_bucket(tk: str, session_day: date, h: int, m: int) -> Optional[float]:
    if pd is None:
        return None
    s = series_for_day_1m(tk, session_day)
...
def _intraday_completely_empty(session_day: date) -> bool:
    if pd is None:
        return True
    if _intraday_cache_day != session_day:
        return True
    for series in _intraday_cache.values():
        if series is not None and not getattr(series, "empty", True):
            return False
    return True
...
class MainWindow(QMainWindow):
    ...
    def _selection_changed(self, current: Optional[QTableWidgetItem], previous: Optional[QTableWidgetItem]) -> None:
        ...
        if row < 0 or row >= len(TICKERS):
            return
        tk = TICKERS[row]
        series = series_for_day_1m(tk, self.session_date)
    ...
    def _build_capture_payload(
        ...
        for tk in TICKERS:
            snap_val = prev_snapshot.get(tk)
            prev_map[tk] = float(snap_val) if isinstance(snap_val, (int, float)) else None
        ...
        try:
            ...
            intraday_empty = _intraday_completely_empty(session_day)
            ...
            if intraday_empty:
                fallback_label = "4:00 PM"
                h, m = BUCKETS[LABEL_TO_INDEX[fallback_label]][1]
                price_map: Dict[str, Optional[float]] = {}
                for tk in TICKERS:
                    fallback_val = last_close(tk)
                    if fallback_val is None:
                        snap_val = prev_snapshot.get(tk)
                        fallback_val = float(snap_val) if isinstance(snap_val, (int, float)) else None
                    if isinstance(fallback_val, (int, float)):
                        payload["prev_close"][tk] = float(fallback_val)
                        price_map[tk] = float(fallback_val)
                    else:
                        payload["prev_close"][tk] = None
                        price_map[tk] = None
                        stale.add(tk)
                ...
                payload["source"] = "last_close_only"
                return payload
    ...
    def _handle_capture_result(self, fut: Future, force_export: bool) -> None:
        ...
        if last_label:
            ...
            if self._last_cache_source == "last_close_only":
                msg = f"Captured {last_label} · last-close only (no intraday data for today)"
            else:
    ...
    def _export_excel_grid_and_bucket(self, label: str) -> None:
        if openpyxl is None or load_workbook is None:
            return
        try:
        ...
        try:
            ...
            fn = DATA_DIR / day_file_name(day_dt)
            grid_name = "Grid"
            ...
            if fn.exists():
                ...
                for sheet in list(wb.worksheets):
                    wb.remove(sheet)
                wb.create_sheet(grid_name)

            if grid_name not in wb.sheetnames:
                wb.create_sheet(grid_name)
            grid_ws = wb[grid_name]

            headers = ["Ticker"] + TIME_COLS
            for col_idx, header in enumerate(headers, start=1):
                cell = grid_ws.cell(row=1, column=col_idx)
                cell.value = header
                cell.font = Font(bold=True)

            for row_idx, tk in enumerate(TICKERS, start=2):
                name_cell = grid_ws.cell(row=row_idx, column=1)
                name_cell.value = f"{row_idx-1}. {tk}"
                name_cell.font = Font(bold=True, color="000000FF" if tk in ("INTC", "WBA") else "00000000")
                for col_idx, bucket_label in enumerate(TIME_COLS, start=2):
                    price_val = self.bucket_prices.get(tk, {}).get(bucket_label)
                    base = self._base_for(tk, bucket_label)
                    text, pct, _ = self._render_cell(tk, bucket_label, price_val, base)
                    grid_ws.cell(row=row_idx, column=col_idx, value=text)
                    grid_ws.cell(row=row_idx, column=col_idx).fill = PatternFill(
                        "solid", fgColor=self._excel_color_for(pct)
                    )

            bucket_ws = wb[bucket_name] if bucket_name in wb.sheetnames else wb.create_sheet(bucket_name)
            bucket_headers = ["ticker", "price", "pct"]
            for col_idx, header in enumerate(bucket_headers, start=1):
                cell = bucket_ws.cell(row=1, column=col_idx)
                cell.value = header
                cell.font = Font(bold=True)

            for row_idx, tk in enumerate(TICKERS, start=2):
                price_val = self.bucket_prices.get(tk, {}).get(label)
                base = self._base_for(tk, label)
                pct_val: Optional[float] = None
                if isinstance(price_val, (int, float)) and isinstance(base, (int, float)) and base:
                    pct_val = ((price_val / base) - 1.0) * 100.0
                bucket_ws.cell(row=row_idx, column=1, value=f"{row_idx-1}. {tk}")
                bucket_ws.cell(row=row_idx, column=2, value=price_val if isinstance(price_val, (int, float)) else None)
                bucket_ws.cell(row=row_idx, column=3, value=pct_val)
                fill = PatternFill("solid", fgColor=self._excel_color_for(pct_val))
                bucket_ws.cell(row=row_idx, column=2).fill = fill
                bucket_ws.cell(row=row_idx, column=3).fill = fill

            extra_rows = bucket_ws.max_row - (len(TICKERS) + 1)
            if extra_rows > 0:
                bucket_ws.delete_rows(len(TICKERS) + 2, extra_rows)
